  ...
  return (
    <div className="min-h-screen bg-brand-grafite text-brand-creme">
      <header className="bg-brand-surface border-b border-brand-border px-6 py-4 flex items-center gap-4">
        <Link to="/admin" className="flex items-center gap-1.5 text-brand-muted hover:text-brand-creme transition-colors text-sm">
          <ArrowLeft size={16} />
          Voltar ao Painel
        </Link>
      ...
      <form onSubmit={handleSalvar} className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            >
              ...
              <optgroup label="📌 Seções e Editoriais do Portal">
                {secoesEditoriais.map(cat => (
                  <option key={cat.id} value={cat.id}>{cat.nome}</option>
                ))}
              </optgroup>
              <optgroup label="🏙️ Municípios do Semiárido">
        ...
        <details className="rounded-xl bg-brand-surface border border-brand-border p-4">
          <summary className="cursor-pointer text-sm font-semibold text-brand-muted">⚙️ SEO Avançado (Opcional)</summary>
